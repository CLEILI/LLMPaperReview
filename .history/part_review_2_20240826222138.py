...
def review(pdf:list,retrieval_function):
    ...
    user_proxy = UserProxyAgent(
        ...
        is_termination_msg=lambda msg: "{\"comment\":" in str(msg["content"]) or "Thank" in str(msg["content"]),
    )
    ...
    executor=user_proxy,
    name="QuestionAnswer",
    description=f"useful when you want to answer questions about the paper named in {pdf}, except the score.",
)
    '''register_function(
    ...
    At last, you must reply every paper's information in the format of the following examples:
